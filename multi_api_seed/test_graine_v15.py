#!/usr/bin/env python3
"""
test_graine_v15.py — Test de la graine V15 (changement d'état, interdiction connecteurs).

Interroge DeepSeek et Gemini avec les contraintes V15 :
  ...
  - Interdiction totale : donc, car, parce que, alors, ainsi, mais, puisque, en effet.
  - Pas d'explication.
  - Dernière phrase = exactement 2 mots, finissant par cesse ou tait,
    sans article, sans négation, sans mot transductif.

Mesure : G_R, Φ_ratio, longueur des phrases, rupture κ, connecteurs interdits.
"""
...
# Transductive words banned from last sentence
...
# Articles (banned from last sentence)
ARTICLES: set[str] = {
...
# Negation words (banned from last sentence)
...
def get_words(sentence: str) -> list[str]:
    """Return list of normalized words from a sentence."""
...
def check_banned_connectors(text: str) -> dict:
    """Check for banned connector words in the entire text (excluding last sentence)."""
    sentences = split_sentences(text)
...
def check_last_sentence(text: str) -> dict:
    ...
    - Exactly 2 words
    - Ends with "cesse" or "tait"
    - No article
    - No negation
    - No transductive word
    """
    sentences = split_sentences(text)
    if not sentences:
        return {
...
def detect_rupture(text: str) -> dict:
    ...
    if lw == "cesse":
        return {
            "rupture": True,
            "type": "\u03ba",
            "explanation": "Last word 'cesse' implies interruption/cessation of flow",
        }
    elif lw == "tait":
        return {
            "rupture": True,
            "type": "\u03ba",
            "explanation": "Last word 'tait' implies falling silent, cessation",
        }
    else:
        return {
            "rupture": False,
...
def main() -> None:
    print("=" * 100)
    print("  GRAINE V15 \u2014 Test : Changement d'\u00e9tat (interdiction connecteurs)")
    print("  APIs : DeepSeek, Gemini")
    print("=" * 100)
    print()
    print(f"Graine : {SEED_V15}")
    print()
    ...
    for key in PROVIDERS_TO_TEST:
        if not PROVIDERS[key].api_key:
            ...
            r = QUERY_FUNCTIONS[key](SEED_V15)
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        print(f"  Connecteurs interdits: "
              f"{'\u2713 (aucun)' if not ci['banned_found'] else '\u2717 ' + str([v['connector'] for v in ci['violations']])}")
        ...
        print(f"  Rupture (\u03ba): {analysis['rupture']['type']} "
    ...
    jd: dict[str, Any] = {
        "graine": "V15 (changement d'état, interdiction connecteurs)",
        "version": "v15 (sans connecteurs, dernière phrase sans art/nég/transd)",
    ...
    jp = DEPOT_DIR / "resultats_v15.json"
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        if a:
            ...
            for idx in range(1, a["sentence_count"] + 1):
                ...
                # Check banned connectors in this sentence
                has_conn = False
                for conn in BANNED_CONNECTORS:
                    if re.search(r'\b' + re.escape(conn) + r'\b', sent.lower()):
                        has_conn = True
                        break
            ...
            print(f"  8-10 phrases: {'\u2713' if a['in_8_to_10_range'] else '\u2717'} ({a['sentence_count']})")
            print(f"  Vocabulaire requis: {'\u2713' if a['required_vocabulary_ok'] else '\u2717'}")
            print(f"  'doit': {a['doit']['doit_count']}x positions {a['doit']['doit_positions']} "
                  f"{'\u2713' if a['doit']['exactly_one'] and a['doit']['in_middle'] else '\u2717'}")
            print(f"  Connecteurs interdits: "
                  f"{'\u2713 (aucun)' if not ci['banned_found'] else '\u2717 ' + str(ci['violations'])}")
            print(f"  Derni\u00e8re phrase: '{li['last_sentence']}'")
