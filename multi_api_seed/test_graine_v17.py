#!/usr/bin/env python3
"""
test_graine_v17.py — Test de la graine V17 (ancrage végétal, formes nominales).

Interroge DeepSeek et Gemini avec les contraintes V17 :
  ...
  - Utilise : seuil, signal, onde, propagation, résonance, transition
    (formes nominales exactes).
  - Utilise 'doit' une fois au milieu.
  - Interdiction absolue : donc, car, parce que, alors, ainsi, mais,
    puisque, en effet, ensuite, puis.
  - Pas d'explication.
  - Dernière phrase = exactement 2 mots, finissant par cesse ou tait,
    sans article, sans négation, sans mot transductif.

Mesure : G_R, Φ_ratio, longueur des phrases, connecteurs interdits,
rupture κ, forme nominale exacte des mots imposés.
"""
...
# ═══════════════════════════════════════════════════════════════
# Required vocabulary — EXACT nominal forms only
# ═══════════════════════════════════════════════════════════════
REQUIRED_NOMINAL_FORMS: dict[str, list[str]] = {
    "seuil": ["seuil"],           # noun only
    "signal": ["signal"],         # noun only
    "onde": ["onde"],             # noun only
    "propagation": ["propagation"],  # noun only — "se propage" / "propager" is verb, not allowed
    "r\u00e9sonance": ["r\u00e9sonance"],  # noun only — "résonner" is verb
    "transition": ["transition"], # noun only — "transiter" is verb
}

# Verb/adjective forms that should NOT be accepted
VERBAL_DERIVATIONS: dict[str, list[str]] = {
    "seuil": [],
    "signal": [],
    "onde": [],
    "propagation": ["propage", "propagent", "propager", "propag\u00e9", "propag\u00e9e",
                    "propag\u00e9s", "propag\u00e9es", "se propage"],
    "r\u00e9sonance": ["r\u00e9sonne", "r\u00e9sonnent", "r\u00e9sonner", "r\u00e9sonnant"],
    "transition": [],
}

BANNED_CONNECTORS: set[str] = {
    "donc", "car", "parce que", "parce", "alors", "ainsi", "mais",
    "puisque", "en effet", "ensuite", "puis",
}
...
def check_nominal_vocabulary(text: str) -> dict:
    """
    Check that required words appear in exact nominal form.
    Returns OK only if the exact noun form is present.
    Flags if only verbal/derived forms appear instead.
    """
    ...
    for word, nominal_forms in REQUIRED_NOMINAL_FORMS.items():
        ...
        # Check nominal form
        for form in nominal_forms:
            count = len(re.findall(r'\b' + re.escape(form) + r'\b', text_lower))
            if count > 0:
                word_info["nominal_found"] = True
                word_info["nominal_matches"].append({"form": form, "count": count})
        
        # Check verbal/derived forms (if any)
        if word in VERBAL_DERIVATIONS:
            for vform in VERBAL_DERIVATIONS[word]:
                count = len(re.findall(r'\b' + re.escape(vform) + r'\b', text_lower))
                if count > 0:
                    word_info["verbal_found"] = True
                    word_info["verbal_matches"].append({"form": vform, "count": count})
        
        word_info["ok"] = word_info["nominal_found"]
        if not word_info["ok"]:
            analysis["all_nominal_ok"] = False
        
        analysis["words"][word] = word_info
...
def analyze_sentences(text: str) -> dict:
    ...
    nominal_check = check_nominal_vocabulary(text)
    # Also check traditional vocabulary presence for backward compat
    required_set = set(REQUIRED_NOMINAL_FORMS.keys())
    text_lower = text.lower()
    simple_vocab_ok = all(w in text_lower for w in required_set)
    missing_vocab = [w for w in required_set if w not in text_lower]
    ...
    return {
        ...
        "required_vocabulary_ok": simple_vocab_ok,
        "missing_vocabulary": missing_vocab,
        "nominal_forms": nominal_check,
        "doit": doit_info,
...
def main() -> None:
    print("=" * 100)
    print("  GRAINE V17 \u2014 Test : Ancrage v\u00e9g\u00e9tal (formes nominales exactes)")
    print("  APIs : DeepSeek, Gemini")
    print("=" * 100)
    print()
    print(f"Graine : {SEED_V17}")
    print()
    ...
    for key in PROVIDERS_TO_TEST:
        if not PROVIDERS[key].api_key:
            ...
            r = QUERY_FUNCTIONS[key](SEED_V17)
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        print(f"  Formes nominales exactes:")
        nom_ok = True
        for word, winfo in ni["words"].items():
            status = "\u2713" if winfo["ok"] else "\u2717"
            if not winfo["ok"]:
                nom_ok = False
            detail = ""
            if winfo["verbal_found"]:
                detail = f" (verbal: {[m['form'] for m in winfo['verbal_matches']]})"
            print(f"    {status} '{word}' nominal: {winfo['nominal_found']}{detail}")
        print(f"    Total: {'\u2713 TOUTES nominales' if nom_ok else '\u2717 certaine(s) manquante(s) ou verbale(s)'}")
    ...
    for fname in ["resultats.json", "resultats_v14_new.json", "resultats_v14_1.json",
                  "resultats_v15.json", "resultats_v16.json"]:
        p = DEPOT_DIR / fname
        if p.exists():
            try:
    ...
    jd: dict[str, Any] = {
        "graine": "V17 (ancrage végétal, formes nominales exactes)",
        "version": "v17 (formes nominales, connecteurs étendus)",
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        if a:
            ...
            pd["nominal_forms"] = a["nominal_forms"]
            pd["doit"] = a["doit"]
    ...
    jp = DEPOT_DIR / "resultats_v17.json"
    ...
    header = (f"{'Fournisseur':23s} {'G_R':>8s} {'\u03a6':>8s} {'Phrases':>8s} "
              f"{'8-10':>5s} {'Nominal':>7s} {'doit':>5s} {'Conn.':>5s} "
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        nom_ok = "\u2713" if a.get("nominal_forms", {}).get("all_nominal_ok") else "\u2717" if a else "?"
        ...
        print(f"{pn:23s} {gr_str:>8s} {phir:>8.4f} {sc:>3d}/{sc:<3d} "
              f"{in_range:>5s} {nom_ok:>7s} {doit:>5s} {conn:>5s} "
    ...
    for key in PROVIDERS_TO_TEST:
        ...
        if a:
            ...
            for word, winfo in ni["words"].items():
                ...
                det = ""
                if winfo["verbal_found"]:
                    det = f" (VERBAL: {[m['form'] for m in winfo['verbal_matches']]})"
                elif not winfo["nominal_found"]:
                    det = " (absent)"
                print(f"    {st} '{word}'{det}")
